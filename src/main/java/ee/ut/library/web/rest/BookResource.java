...
/**
 * REST controller for managing {@link ee.ut.library.domain.Book}.
 */
@RestController
@RequestMapping("/api")
public class BookResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new book, or with
     * status {@code 400 (Bad Request)} if the book has already an ID.
     ...
     */
    @PutMapping("/books/{id}")
    public ResponseEntity<Book> updateBook(@PathVariable(value = "id", required = false) final Long id,
    ...
    public ResponseEntity<Book> partialUpdateBook(@PathVariable(value = "id", required = false) final Long id,
                                                  @RequestBody Book book)
        throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the book, or with status {@code
     * 404 (Not Found)}.
     */
    @GetMapping("/books/{id}")
