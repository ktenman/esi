...
import ee.ut.library.management.SecurityMetersService;
...
public class TokenProvider {
        ...
        if (!ObjectUtils.isEmpty(secret)) {
            ...
            log.warn(
                "Warning: the JWT key used is not Base64-encoded. " +
                    "We recommend using the `jhipster.security.authentication.jwt.base64-secret` key for optimum " +
                    "security."
            );
    ...
    public boolean validateToken(String authToken) {
        try {
            ...
            IllegalArgumentException e) { // TODO: should we let it bubble (no catch), to avoid defensive programming
