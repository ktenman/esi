...
describe('Book e2e test', () => {
  ...
  describe('new Book page', () => {
    ...
    it('should create an instance of Book', () => {
      cy.get(`[data-cy="author"]`).type('Oregon zero').should('have.value', 'Oregon zero');

      cy.get(`[data-cy="title"]`).type('hardware redundant').should('have.value', 'hardware redundant');

      cy.get(`[data-cy="releaseDate"]`).type('2022-05-10').should('have.value', '2022-05-10');

      cy.get(`[data-cy="status"]`).select('RENTED');

      cy.get(`[data-cy="category"]`).select('DRAMA');

      cy.get(`[data-cy="language"]`).select('ESTONIAN');
